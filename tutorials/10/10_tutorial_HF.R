...
df_s["edu_cat"] <- NA ##Creating an empty column of NAs and then step by step replacing these values
...
View(df)
is.factor(df$edu_cat) ##If you reload the data, remember to recode it as factor variable
...
vioplot(df$euftf_re ~ df$edu_cat) ##violin plot
boxplot(df$euftf_re ~ df$edu_cat)
plot(df$edlvdie,df$euftf_re) #Euroskepticism is 0-10 but we treat this categorical variable as quasi-continuous(need to explain this decision)
##based on 2 criteria: 1) No. of levels: if the scale has equidistance. 
##2)
plot(jitter(df$edlvdie,2),jitter(df$euftf_re,2)) ##Reasonable way of using jitter
...
#Euroskepticism = 4.48 - 0.027 * education level + error term (?)
...
# If we dont have theoretical reasons, we make the category with the highest observations as reference category
plot(df$edu_cat)
